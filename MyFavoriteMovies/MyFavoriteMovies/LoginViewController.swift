...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard (error == nil) else {
                displayError("We have an error while fetching data")
                return
            }
            ...
            do{
                ...
                displayError("We have an error while parsing data")
                return
            }
            guard let requestToken = parsedData[Constants.TMDBResponseKeys.RequestToken] as? String else {
                displayError("We have an error while fetching requestToken")
                return
            }
            /* 6. Use the data! */
    ...
    private func loginWithToken(requestToken: String) {
        ...
        let postData = NSData(data: "{\(Constants.TMDBParameterKeys.Username):\(self.usernameTextField.text),\(Constants.TMDBParameterKeys.Password):\(passwordTextField.text) }".dataUsingEncoding(NSUTF8StringEncoding)!)
        
        /* 1. Set the parameters */
        let methodParameters = [
            Constants.TMDBParameterKeys.ApiKey: Constants.TMDBParameterValues.ApiKey,
            Constants.TMDBParameterKeys.RequestToken: requestToken as String!,
       ...
       /* 4. Make the request */
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard (error == nil) else {
                displayError("We have an error while fetching session ID")
                return
            }
            
            guard let data = data else{
                displayError("Session ID Data not found")
                return
            }
            /* 5. Parse the data */
            ...
            do{
                ...
                displayError("We have an error while parsing session ID data")
                return
            }
            
            guard let successKey = parsedData[Constants.TMDBResponseKeys.Success] as? Bool where successKey ==  true else{
    ...
    private func getSessionID(requestToken: String) {
        ...
        /* 2/3. Build the URL, Configure the request */
        let request = NSURLRequest(URL: appDelegate.tmdbURLFromParameters(methodParameters as [String:AnyObject], withPathExtension: "/authentication/session/new"))
    ...
    private func getUserID(sessionID: String) {
        ...
        let methodParameters = [
            Constants.TMDBParameterKeys.ApiKey: Constants.TMDBParameterValues.ApiKey,
            Constants.TMDBParameterKeys.SessionID: appDelegate.sessionID as String!
        ]
        /* 2/3. Build the URL, Configure the request */
